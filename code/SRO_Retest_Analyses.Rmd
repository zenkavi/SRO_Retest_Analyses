...
The literature does a better job in predicting itself for tasks compared to surveys. s
...
tmp = pred_sample_out %>%
  ...
  mutate(diff = (pred - value)^2)

tmp%>%  
  ggplot(aes(diff, fill=key))+
...
Are the means of these distributions different? Yes: the difference is larger when predicting the left-out value from the literature versus predicting the left-out average from our data

```{r}
with(tmp, t.test(diff ~ key))
```

Plot difference between prediction and value broken down by measure type.
Looks like there might be an interaction between the measure type and whether the left out sample is a value from the literature versus the average from our data.
The difference between the prediction and left value seems smaller on average for survey measures compared to survey measures.
Moreover, for survey measures predictions of the literature are better compared to predictions of our data but the opposite is true for task measures.

```{r}
tmp%>%
  ggplot(aes(diff, fill=key, color=task, alpha=task))+
  geom_density(position = "identity", size=2)+
  scale_fill_manual(breaks=c("mean_pearson","retest_reliability"),
                       labels=c("Empirical", "Literature"),
                      name="Predicting",
...
Can we capture this interaction in a single model?

```{r}
summary(lm(value ~ key*task,tmp))
```

Data versus model prediction plot

```{r}

```

Distributions of sum squared residual
...
tmp2 %>%
  ...
  ggplot(aes(factor(raw_fit, levels = c("raw", "EZ", "hddm"), labels=c("Raw", "EZ-diffusion", "Hierarchical diffusion")), icc, fill=factor(rt_acc, levels = c("rt","accuracy", "drift rate", "threshold", "non-decision"), labels=c("Response Time", "Accuracy","Drift Rate", "Threshold", "Non-decision"))))+
  geom_boxplot()+
  facet_wrap(~factor(overall_difference, levels=c("overall", "difference"), labels=c("Overall (all trials)", "Difference (contrasting conditions)")))+
  theme_bw()+
  ylab("ICC")+
  xlab("")+
  theme(legend.title = element_blank(),
