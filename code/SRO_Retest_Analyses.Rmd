...
```{r}
summary(lm(value~key*similarity,tmp))
effect.size(lm(value~key*similarity,tmp))
```

```{r}
summary(MCMCglmm(value ~ key*similarity, data=tmp, nitt = 50000)) 
```
...
Our data is better in predicting the literature estimates for the surveys that it is in predicting tasks (comparing the difference between orange and purple for red and blue outlines).

```{r}
with(comp_lit_pred_out_both, t.test(r2_lit-r2_boot ~ task))
```

Why are the reliability estimates from surveys worse compared to tasks in predicting the literature?
...
In this procedure having high variance is better. When predicting the literature using an empirical estimate for a survey measure
