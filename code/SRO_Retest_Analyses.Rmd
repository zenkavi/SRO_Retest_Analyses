...
```{r}
rel_comp = rel_comp %>%
  mutate(diff_sq = (mean_icc - retest_reliability)^2) 
...
```{r}
with(rel_comp %>% filter(task == "task"), cor(mean_icc, retest_reliability))
```

```{r}
with(rel_comp %>% filter(task == "survey"), cor(mean_icc, retest_reliability))
```
...
```{r}
mod4 = lmer(retest_reliability ~ mean_icc + task + scale(sample_size) + (mean_icc|dv), data=rel_comp)
...
How would I show random intercepts/slopes?
