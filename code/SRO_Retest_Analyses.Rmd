...
`Plot differences for the measures with differences as examples`
...
Conceptual replication 1: Compare effect sizes of effect of task on reliability estimates in literature vs our results (just looking at the variables you find in the literature)

```{r}
m1 = rel_comp %>% filter(task == "task") %>% summarise(m1 = mean(retest_reliability))
m2 = rel_comp %>% filter(task == "survey") %>% summarise(m2 = mean(retest_reliability))
n1 = nrow(rel_comp %>% filter(task == "task"))
n2 = nrow(rel_comp %>% filter(task == "survey"))
var1 = rel_comp %>% filter(task == "task") %>% summarise(var1 = var(retest_reliability))
var2 = rel_comp %>% filter(task == "survey") %>% summarise(var1 = var(retest_reliability))
sd_pooled = sqrt(((n1-1)*var1+(n2-1)*var2)/(n1+n2-2))
d_lit = abs(m1-m2)/sd_pooled
d_lit
```

The literature review conceptually replicates our finding of lower reliability estimates for task measures but with a much smaller effect size.
...
boot_comp = boot_comp %>%
  mutate(task=ifelse(grepl("survey", dv), "task", "survey"))

m1 = boot_comp %>% filter(task == "task") %>% summarise(m1 = mean(icc))
m2 = boot_comp %>% filter(task == "survey") %>% summarise(m2 = mean(icc))
n1 = nrow(boot_comp %>% filter(task == "task"))
n2 = nrow(boot_comp %>% filter(task == "survey"))
var1 = boot_comp %>% filter(task == "task") %>% summarise(var1 = var(icc))
var2 = boot_comp %>% filter(task == "survey") %>% summarise(var1 = var(icc))
sd_pooled = sqrt(((n1-1)*var1+(n2-1)*var2)/(n1+n2-2))
d_boot = abs(m1-m2)/sd_pooled
d_boot
```

Conceptual replication 2: Larger variance in reliability estimates for tasks

```{r}
tmp = rel_comp %>%
  group_by(dv) %>%
  summarise(rel_var = var(retest_reliability), 
            task=unique(task)) %>%
  na.omit()
tmp %>%
  ggplot(aes(rel_var, fill=task))+
  geom_density(alpha=0.5, position='identity')+
  theme(legend.title = element_blank())+
  xlab('Variance of reliability estimates')
```

```{r}
summary(lm(rel_var ~ task, tmp))
```

Effect sizes of task on variance in reliability estimates in literature vs our results are comparable

```{r}
m1 = tmp %>% filter(task == "task") %>% summarise(m1 = mean(rel_var))
m2 = tmp %>% filter(task == "survey") %>% summarise(m2 = mean(rel_var))
n1 = nrow(tmp %>% filter(task == "task"))
n2 = nrow(tmp %>% filter(task == "survey"))
var1 = tmp %>% filter(task == "task") %>% summarise(var1 = var(rel_var))
var2 = tmp %>% filter(task == "survey") %>% summarise(var2 = var(rel_var))
sd_pooled = sqrt(((n1-1)*var1+(n2-1)*var2)/(n1+n2-2))
d_lit = abs(m1-m2)/sd_pooled
d_lit
```
...
Square difference between our mean estimate and the reliability from the literature decreases exponentially with sample size. The smaller the sample size in the literature the more the reliability estimate differs from our results.

```{r}
rel_comp = rel_comp %>%
  mutate(diff_sq = abs(mean_icc - retest_reliability)^2) 

rel_comp %>%
  ggplot(aes(sample_size,diff_sq, color=task))+
  # geom_point()+
  #exponential decay (linearized by taking logs of both sides)
  geom_smooth(method="glm",se=FALSE, method.args=list(family=gaussian(link="log")))+
  theme_bw()
```
...
```{r}
with(rel_comp %>% filter(task == "task"), cor(mean_icc, mean_reliability))
```

```{r}
with(rel_comp %>% filter(task == "survey"), cor(mean_icc, mean_reliability))
```

Model comparisons building model to predict the reliabilities in the literature from our results.

```{r}
mod1 = lmer(retest_reliability ~ mean_icc + (1|dv), data=rel_comp)
mod2 = lmer(retest_reliability ~ mean_icc + (mean_icc|dv), data=rel_comp)

anova(mod1, mod2)
```

```{r}
mod3 = lmer(retest_reliability ~ mean_icc + scale(sample_size) + (mean_icc|dv), data=rel_comp)

anova(mod2, mod3)
```

```{r}
mod4 = lmer(retest_reliability ~ mean_icc + scale(days) + scale(sample_size) + (mean_icc|dv), data=rel_comp)

anova(mod3, mod4)
```

Selected model: fixed effect for mean icc and sample size of study, random slope with respect to mean icc and random intercept for each measure
...
How well does the model fit? (squared correlation between predictions of model and data)

```{r}
cor(predict(mod3), rel_comp$retest_reliability)^2
```

Same result if you do flat model without random slopes for dv's

```{r}
summary(lm(retest_reliability ~ mean_icc+scale(sample_size), data=rel_comp))
```

Visualize hierarchical model:
How would I show random intercepts/slopes? Does the fact that I can't think of how means that there is something wrong with the model?

```{r}
rel_comp %>%
  ggplot()+  
  geom_boxplot(aes(mean_icc, retest_reliability, group=dv))+
  geom_smooth(aes(mean_icc, retest_reliability), method= 'lm')+
  theme_bw()+
  theme(legend.position = 'none')+
  geom_abline(slope=1, intercept=0)+
  # geom_abline(slope=0, intercept=0.3, linetype='dashed')+
  xlim(-0.2, 1)+
  ylim(-0.2, 1)+
  facet_grid(task~.)+
  xlab('Mean bootstrapped ICC')+
  ylab('Reliability estimate from literature')
