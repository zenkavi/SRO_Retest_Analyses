...
t1_dvs = t1_dvs %>% mutate(sub_id = as.character(sub_id))
t2_dvs = t2_dvs %>% mutate(sub_id = as.character(sub_id))

unique(t2_dvs$sub_id)

t1_dvs[,c("acc","sub_id", "breaks")] %>%
  left_join(t2_dvs[,c("acc","sub_id", "breaks")], by = c("sub_id", "breaks")) %>%
  distinct()
```
