...
Are the means of these distributions different than 0? Or more specifically are each of these distributions different than distributions with mean 0 an sd 1?
When calculating the difference scores I scaled them to have an sd of 1 but did not center them to have a mean of 0. So if there is a meaningful difference the mean of the distribution would differ from 0.

```{r}
diff_from_zero = function(df, samples=100){
  
  one_diff = function(df){
    comp_vec = rnorm(nrow(df))
    test = t.test(df$difference, comp_vec, var.equal = T) #both should have var=1
    out = data.frame(t_val = test$statistic, df = test$parameter, p_val = test$p.value)
    out = out %>% mutate(sig = ifelse(p_val < 0.05, 1, 0))
    return(out)
  }
  return(plyr::rdply(samples, one_diff(df)))
}

diff_from_zero_df = t1_2_difference %>%
  group_by(dv) %>%
  do(diff_from_zero(.))

diff_from_zero_summary = diff_from_zero_df %>%
  summarise(sum_sig=sum(sig)) %>%
  filter(sum_sig>5) %>% 
  arrange(sum_sig) %>%
...
diff_from_zero_summary
```

Chi sq test to see if survey/task has an effect on number of variables with sig difference: It doesn't. So comparable proportions of survey and task variables show systematic differences between the two time points (note that this doesn't necessarily whether the performance has improved or not).

```{r}
sig_survey = (diff_from_zero_summary %>% filter(task=="survey"))$n
sig_task = (diff_from_zero_summary %>% filter(task=="task"))$n
chisq.test(data.frame(survey = c(sig_survey, 74-sig_survey), task=c(sig_task, 366-sig_task)))
```

```{r}
rm(sig_survey, sig_task, diff_from_zero_summary)
```

Before I was using subjects as a random effect but days between the two time points for each measure depends on subj id. What varies randomly is which dv we are looking for its distribution of differences in relation to the days between the time points. So I changed the model to have fixed effect for the days between and a random effect for dependent variable.

```{r}
summary(lmerTest::lmer(difference ~ scale(days_btw)+(scale(days_btw) | dv), data=t1_2_difference)) 
```

But if I just run one mixed effects model then we don't get a sense of the simple effects. I can run it separately for each dv to see if comparable proportions show a dependence on the delay for surveys and tasks.

```{r}
get_delay_effect

df = t1_2_difference %>% filter(dv == "adaptive_n_back.EZ_drift")

mod = lm(difference ~ scale(days_btw), data = df)

str(summary(mod))
mod$coefficients['scale(days_btw)']

data.frame(estimate=coef(summary(mod))["dv","Estimate"], tval=coef(summary(mod))["dv","t value"])
...
Instead I now plot all the data coloring by dependant variable.

```{r}
t1_2_difference %>%
  ggplot(aes(as.numeric(days_btw), difference,col=dv))+
  geom_smooth(method='lm', show.legend = FALSE, se=FALSE)+
  theme(legend.position = 'none')+
  theme_bw()
```

Does the delay between the two time points make a difference in how different the performance at each time point is from each other. Is the difference larger depending on the delay

Is a measure less reliable if the average delay between the two time points is larger? 

### Measure level

For each variable does the average difference between two time points vary by the average number of days between the two time points? 

The average number of days between two measurements would be the same for all measures with current way of coding the days between completion. Even if I were to go in and see if there is a way to figure out the exact day of completion the change would not be more than 7 days since we gave subjects a week to complete. 
...
Plus what you would be interested (y) is not exactly the difference between the measurements from two time points but rather the reliability of each measure.
...
```{r}
sjt.df(tmp_save %>% mutate_if(is.numeric, funs(round(., 3))), describe=F, hide.progress = TRUE, show.rownames = FALSE, file = "/Users/zeynepenkavi/Dropbox/PoldrackLab/SRO_Retest_Analyses/output/figures/var_breakdown_summary.doc")
```
...
sjt.df(tmp %>% mutate_if(is.numeric, funs(round(., 3))), describe=F, hide.progress = TRUE, show.rownames = FALSE, file = "/Users/zeynepenkavi/Dropbox/PoldrackLab/SRO_Retest_Analyses/output/figures/task_rel_table.doc")
```
...
sjt.df(tmp_save %>% mutate_if(is.numeric, funs(round(., 3))), describe=F, hide.progress = TRUE, show.rownames = FALSE, file = "/Users/zeynepenkavi/Dropbox/PoldrackLab/SRO_Retest_Analyses/output/figures/ddm_rel_table.doc")
```
...
The type of ICC we have chosen does not take within subject (between session/systematic) variance in to account. This is why Weir recommends checking whether there is a significant change based on time and examining the SEMs.
...
To check whether a measure shows learning effects in a meaningful number of bootstrapped samples we can: check if the effect of time is significant in each bootstrapped sample and filter variables that have more than 5% of the boostrapped samples showing significant time effects.

Another way might be to compute confidence intervals using SEMs as described in the second half of Weir (2005) and check what percent of participants have scores that fall out of this range. I haven't pursued this at least for now. 

Here we ask: Which variables have significant time effects in more than 5% of the bootstrapped samples?

22/74 survey measures
135/335 task measures
...
sjt.df(tmp %>% mutate_if(is.numeric, funs(round(., 3))), describe=F, hide.progress = TRUE, show.rownames = FALSE, file = "/Users/zeynepenkavi/Dropbox/PoldrackLab/SRO_Retest_Analyses/output/figures/survey_rel_table.doc")
```
...
boot_lit_rel = function(df, samples = 100, rel_col = 'retest_reliability'){
  
  boot_once = function(df){
    #get number of rows in df
    data_amount = nrow(df)
    #sample as many reliability estimates as there are rows in the df
    boot_sample = sample(c(1:data_amount), size=data_amount, replace=TRUE)
    boot_sample = df[boot_sample,]
    #calculate median reliability estimate for the sampled estimates
    return(median(boot_sample[,rel_col])) 
  }
  
  out = replicate(samples, boot_once(df))
  
  return(data.frame(median_lit_rel=out))
}

var_lit_review = lit_review %>%
  group_by(dv)%>%
  do(boot_lit_rel(.))
  summarise(median_reliability=median(retest_reliability),
            task = unique(task)) %>%
  mutate(dv = as.character(dv))
...
rel_comp = var_lit_review %>%
  left_join(var_boot_df, by = 'dv') 

rel_comp
```

Correlation between our median estimates from bootstrapped samples and the literature review for task variables
```{r}
with(rel_comp %>% filter(task == "task"), cor(median_icc, median_reliability))
```

```{r}
with(rel_comp %>% filter(task == "survey"), cor(median_icc, median_reliability))
```

Potential problems in this graph: 
- not all metrics are ICC's
- difference in delay between completion of two tests (does it make a difference in the lit review?)

```{r}
rel_comp %>%
  ggplot(aes(median_reliability, median_icc, color=task))+
  geom_point()+
  geom_abline(slope=1, intercept=0)+
  theme_bw()+
  scale_x_continuous(limits = c(-0.25,1), breaks=c(-0.25, 0, 0.25, 0.5, 0.75, 1))+
  scale_y_continuous(limits = c(-0.25,1), breaks=c(-0.25, 0, 0.25, 0.5, 0.75, 1))+
  theme(legend.title = element_blank())+
  xlab('Median reliability from literature')+
  ylab('Median ICC from bootstrapped samples')
```
...
lit_review %>%
  ...
  geom_point()
summary(lmer(retest_reliability~sample_size+task+(1|dv), data=lit_review))

#sd by average sample size
lit_review %>%
  group_by(dv) %>%
  summarise(sd_rel = sd(retest_reliability),
            avg_sample_size = mean(sample_size)) %>%
  ggplot(aes(avg_sample_size, sd_rel))+
  geom_point()+
  xlim(0,200)
```

x = deviance between our data and literature
y = average sample size

```{r}
rel_comp %>%
  mutate(diff_sq = (median_icc - median_reliability)^2) %>%
  ggplot(aes(diff_sq, mean_sample, color=task))+
  geom_point()+
  geom_smooth()
```
