...
`Plot difference score distributions for the measures with significant differences as examples`

```{r}
t1_2_difference %>% 
  filter(dv == sig_days_effect$dv[1]) %>%
  mutate(days_btw = as.numeric(days_btw),
         days_bins = ifelse(days_btw < quantile(days_btw, probs=0.33), "short", 
                                                ifelse(days_btw>quantile(days_btw, probs=0.66), "large", "medium"))) %>%
  ggplot(aes(factor(days_bins, levels = c("short", "medium", "large")), difference))+
  geom_boxplot()+
  xlab('delay between two measurements')
```

```{r}
t1_2_difference %>% 
  filter(dv == sig_days_effect$dv[4]) %>%
  mutate(days_btw = as.numeric(days_btw),
         days_bins = ifelse(days_btw < quantile(days_btw, probs=0.33), "short", 
                                                ifelse(days_btw>quantile(days_btw, probs=0.66), "large", "medium"))) %>%
  ggplot(aes(factor(days_bins, levels = c("short", "medium", "large")), difference))+
...
Compare effect sizes of effect of task on reliability estimates in literature vs our results (just looking at the variables you find in the literature)

```{r}
rel_comp = rel_comp %>%
  mutate(retest_reliability_ft = 0.5*log((1+retest_reliability)/(1-retest_reliability))) %>%
  filter(is.finite(retest_reliability_ft)==TRUE)

m1 = rel_comp %>% filter(task == "task") %>% summarise(m1 = mean(retest_reliability_ft))
m2 = rel_comp %>% filter(task == "survey") %>% summarise(m2 = mean(retest_reliability_ft))
n1 = nrow(rel_comp %>% filter(task == "task"))
n2 = nrow(rel_comp %>% filter(task == "survey"))
var1 = rel_comp %>% filter(task == "task") %>% summarise(var1 = var(retest_reliability_ft))
var2 = rel_comp %>% filter(task == "survey") %>% summarise(var1 = var(retest_reliability_ft))
sd_pooled = sqrt(((n1-1)*var1+(n2-1)*var2)/(n1+n2-2))
d_lit = abs(m1-m2)/sd_pooled
d_lit
```

`Add column for turk vs lab study`

The literature review conceptually replicates our finding of lower reliability estimates for task measures but with a much smaller effect size (for Fisher transformed reliability estimates).
...
boot_comp = boot_comp %>%
  mutate(task=ifelse(grepl("survey", dv), "task", "survey"),
         icc_ft = 0.5*log((1+icc)/(1-icc)))

m1 = boot_comp %>% filter(task == "task") %>% summarise(m1 = mean(icc_ft))
m2 = boot_comp %>% filter(task == "survey") %>% summarise(m2 = mean(icc_ft))
n1 = nrow(boot_comp %>% filter(task == "task"))
n2 = nrow(boot_comp %>% filter(task == "survey"))
var1 = boot_comp %>% filter(task == "task") %>% summarise(var1 = var(icc_ft))
var2 = boot_comp %>% filter(task == "survey") %>% summarise(var1 = var(icc_ft))
sd_pooled = sqrt(((n1-1)*var1+(n2-1)*var2)/(n1+n2-2))
...
This plot (weakly) supports that point that our results diverge most from studies with smaller sample sizes.

```{r}
rel_comp = rel_comp %>%
  mutate(diff_sq = abs(mean_icc - retest_reliability)) 
...
But as the plot suggests these are not significant effects overall.

```{r}
summary(lmer(diff_sq ~ sample_size*task + (1|dv), rel_comp))
```
...
```{r}
with(rel_comp %>% filter(task == "task"), cor(mean_icc, retest_reliability, method="spearman"))
```

```{r}
with(rel_comp %>% filter(task == "survey"), cor(mean_icc, retest_reliability, method="spearman"))
```
...
comp_lit_pred <- function(df){
  ...
  mod_lit = lm(retest_reliability ~ lit_predictor, data=sampled_df)
  mod_boot = lm(retest_reliability ~ mean_icc, data=sampled_df)
  ...
  out = data.frame(r2_lit = summary(mod_lit)$r.squared,
                   ...
                   m_boot = coef(summary(mod_boot))["mean_icc","Estimate"])
...
mod1a = lm(retest_reliability ~ mean_icc, data=rel_comp)

cor(predict(mod1), rel_comp$retest_reliability)^2
cor(predict(mod1a), rel_comp$retest_reliability)^2

# mod2 = lmer(retest_reliability ~ mean_icc + (mean_icc|dv), data=rel_comp)

rel_comp %>%
  select(dv, retest_reliability, mean_icc)
...
NOTE: not all metrics are ICC's

```{r echo=FALSE}
rm(tmp, rel_comp, boot_comp, d_boot, d_lit, mod1, mod2, mod3, mod4, n_df, m1, m2, var1, var2, sd_pooled, task, i, n1, n2)
```
