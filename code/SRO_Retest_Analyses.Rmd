...
mytheme <- gridExtra::ttheme_minimal(
    core = list(fg_params=list(cex = 3.5)),
    colhead = list(fg_params=list(cex = 3.0)),
    rowhead = list(fg_params=list(cex = 3.0)))

task_trial_num_table = tableGrob(trial_num_info %>% 
                                   filter(task == "task") %>% 
                                   ungroup() %>% 
                                   select(-task_group, -task),
                                 theme = mytheme, rows=NULL, cols = NULL,
                                 heights = rep(unit(1.25, "in"), 37))

test_plot = arrangeGrob(p4_t, task_trial_num_table, nrow=1)

ggsave('Task_Boot_w_trialinfo.jpg', plot = test_plot, device = "jpeg", path = "../output/figures/", width = 27, height = 48, units = "in", limitsize = FALSE, dpi = 100)
```
...
task_trial_num_table = task_trial_num %>%
  mutate(y_axis = rev(seq(from=1, to=37, by=1))) %>%
  gather(key, value, -y_axis, -task_group) %>%
  ggplot(aes(key, y_axis))+
...
test_plot = arrangeGrob(p4_t, task_trial_num_table, nrow=1, widths = c(2,1))

ggsave('Task_Boot_w_trialinfo.jpg', plot = test_plot, device = "jpeg", path = "../output/figures/", width = 20, height = 48, units = "in", limitsize = FALSE, dpi = 100)
...
Task level summary of bootstrapped reliabilities.
