...
demog_boot_df %>%
  group_by(dv) %>%
  summarise(median_icc3.k = quantile(icc3.k, probs=0.5),
            icc3.k_2.5 = quantile(icc3.k, probs = 0.025),
            icc3.k_97.5 = quantile(icc3.k, probs = 0.975)) %>%
  arrange(-median_icc3.k)
```

Point estimates of reliability for demog items

```{r}
demog_rel_df = make_rel_df(test_demog, t2_df = retest_demog, metrics=c('icc3.k'))
...
demog_rel_df %>%
  ...
  ggplot(aes(icc3.k, fill = factor(timed)))+
...
r^2 of retest_reliability ~ mean_icc3.k

coef of retest_reliability ~ sampled_reliability vs.
coef of retest_reliability ~ mean_icc3.k
...
Though we are primarily reporting icc3.k's as our metric of reliability the results don't change depending on the metric chosen. Here we plot point estimates of three different reliability metrics against each other (icc3.k, Pearson, Spearman). The bootstrapped version is essentially the same but the plots are busier due to more datapoints.

```{r}
rel_df = make_rel_df(t1_df = test_data, t2_df = retest_data, metrics = c('spearman', 'icc3.k', 'pearson', 'var_breakdown', 'partial_eta', 'sem'))

rel_df$task = 'task'
rel_df[grep('survey', rel_df$dv), 'task'] = 'survey'
rel_df[grep('holt', rel_df$dv), 'task'] = "task"
rel_df = rel_df %>%
  select(dv, task, spearman, icc3.k, pearson, partial_eta, sem, var_subs, var_ind, var_resid)
```
...
```{r}
cor(rel_df[,c('spearman', 'icc3.k', 'pearson')])
```

Note: Some variables have <0 icc3.k's. This would be the case if the $MS_{error}$>$MS_{between}$. Data for these variables have no relationship between the two time points.
...
boot_df %>%
  group_by(dv) %>%
  summarise(icc3.k_median = quantile(icc3.k, probs = 0.5),
            icc3.k_2.5 = quantile(icc3.k, probs = 0.025),
            icc3.k_97.5 = quantile(icc3.k, probs = 0.975),
  ...
  datatable() %>%
  formatRound(columns=c('icc3.k_median', 'icc3.k_2.5', 'icc3.k_97.5', 'spearman_median', 'spearman_2.5', 'spearman_97.5'), digits=3)
```
...
tmp %>%
  datatable() %>%
  formatRound(columns=c('median_icc3.k','icc3.k_2.5','icc3.k_97.5'), digits=3)
```

### Number of trials

Does number of items in a task have a significant effect on the average icc3.k of meaningful measures for all trials from a task? No.
...
summary(icc3.k_by_num_trials_model)
```

```{r warning=FALSE, message=FALSE}
tmp %>%
  ggplot(aes(num_all_trials, icc3.k))+
  geom_point()+
  geom_smooth(method="lm")+
  theme_bw()+
...
tmp_save %>%
  datatable() %>%
  formatRound(columns=c('icc3.k_median', 'icc3.k_2.5', 'icc3.k_97.5'), digits=3)
```

Comparing contrast vs non-contrast: overall has higher reliability than difference.

```{r}
summary(lmerTest::lmer(icc3.k ~ contrast + (1|dv) ,tmp))
# summary(MCMCglmm(icc3.k ~ contrast, random = ~ dv, data=tmp, verbose=FALSE))
```

Comparing raw vs ddm in overall estimates: EZ is significantly better than HDDM and comparable to raw estimates.

```{r}
summary(icc3.k_by_rawfit_noncon_model)
```

Comparing raw vs ddm in difference scores: EZ is significantly worse than HDDM and comparable to raw estimates.

```{r}
summary(icc3.k_by_rawfit_con_model)
```

```{r echo=FALSE}
rm(icc3.k_by_rawfit_con_model, icc3.k_by_rawfit_noncon_model)
```

```{r echo=FALSE, out.width='100%'}
fig_name = 'Bootstrap_DDM_Comp_Legend.jpeg'
...
tmp %>%
  group_by(contrast) %>%
  summarise(mean_icc3.k = mean(icc3.k),
            sd_icc3.k = sd(icc3.k))
```

```{r echo=FALSE, out.width='100%'}
fig_name = 'PointEst_DDM_Comp.jpeg'
...
Does number of items in a survey have a significant effect on the average icc3.k of survey measures? No.

```{r warning=FALSE, message=FALSE}
summary(lmerTest::lmer(icc3.k ~ num_all_trials + (1|dv), data = tmp))
```
