...
t1_2_difference = t1_2_difference %>% separate(dv, c("task", "dv"), sep="\\.")
```
...
```{r}
mod = lmer(difference ~ dv + scale(days_btw) + (1|sub_id), data = t1_2_difference)

data.frame(estimate=coef(summary(mod))["scale(days_btw)","Estimate"], tval=coef(summary(mod))["scale(days_btw)","t value"])
```
...
```{r warning=FALSE, message=FALSE}
tmp = t1_2_difference %>%
  group_by(sub_id, task) %>%
  summarise(mean_diff = mean(difference, na.rm=T),
            sem_diff = sem(difference),
            days_btw = unique(days_btw))
tmp %>%
  ggplot(aes(days_btw, mean_diff))+
  geom_point()+
  geom_errorbar(aes(ymin = mean_diff-(1.96*sem_diff), ymax = mean_diff+(1.96*sem_diff)))+
  theme_bw()+
...
```{r warning=FALSE, message=FALSE}
summary(lm(mean_diff ~ scale(days_btw), data=tmp))
```
