...
get_t_test_out <- function(df){
  out = data.frame(p.value=NA, mean_x = NA, mean_y=NA)
  mod = t.test(df$icc.x, df$icc.y)
...
Plot flat estimates against hierarhical estimates.

Check the distributions of the flat and hierarchical estimates, transform if not normal and t-test to check if they are different than each other (alternatively get difference scores and check if they are different than 0/plot the distribution of the difference scores)
