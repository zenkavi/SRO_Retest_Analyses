...
```{r}
t1_2_difference %>%
  left_join(measure_labels[,c("dv", "overall_difference", "raw_fit", "rt_acc")], by= "dv") %>%
  filter(overall_difference == "overall",
         rt_acc %in% c("rt", "accuracy"),
         task == "task") %>%
  group_by(dv) %>%
  summarise(mean_diff = mean(difference),
            task_name = unique(task_name),
...
tmp = t1_2_difference %>%
  left_join(measure_labels[,c("dv", "overall_difference", "raw_fit", "rt_acc")], by= "dv") %>%
  filter(overall_difference == "overall",
         rt_acc %in% c("accuracy"),
         task == "task") %>%
  group_by(dv) %>%
  summarise(mean_diff = mean(difference),
            task_name = unique(task_name))

t.test(tmp$mean_diff)
```

Write new function to estimate reliability accounting for learning effect
