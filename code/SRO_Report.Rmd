...
hr_merge = merge(t1_dvs, t2_dvs, by = c("worker_id", "breaks"))

# rm(t1_dvs, t2_dvs)

hr_merge = hr_merge %>%
  gather(key, value, -worker_id, -breaks) %>%
...
trial_num_rel_df = data.frame()
for(i in t1_dvs$breaks){
  tmp_t1_dvs = t1_dvs %>% filter(i)
  tmp_t2_dvs = t2_dvs %>% filter(i)
  trial_num_rel_df$breaks[i] = i
