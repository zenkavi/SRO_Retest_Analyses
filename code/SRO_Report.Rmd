...
tmp %>%
  gather(key, value, -dv, -task) %>%
  group_by(task, key) %>%
  ggplot(aes(factor(task, levels=c("task","survey"), labels=c("Task", "Survey")), value, fill=task, alpha=key))+
  geom_boxplot()+
  theme_bw()+
  theme(legend.title = element_blank())+
  scale_alpha_manual(breaks = c("var_subs_pct", "var_ind_pct", "var_resid_pct"),
                    labels = c("Variance between individuals",
                               "Variance between sessions",
                               "Error variance"),
                    values=c(0.1, 0.5, 1))+
  scale_fill_discrete(guide = FALSE)+
  guides(alpha=guide_legend(override.aes=list(fill=hcl(c(15,195),100,0,alpha=c(0.1,0.5, 1)),
                                              colour=NA)))+
  xlab("")+
  ylab("Percent")

ggsave('Variance_Breakdown_Plot_Summary.jpg', device = "jpeg", path = "/Users/zeynepenkavi/Dropbox/PoldrackLab/SRO_Retest_Analyses/output/figures", width = 10, height = 5, units = "in", dpi = 100)
```
...
target task: one without hddm variables, has dv's that are easily codable in r, a lot of trials, possibly multiple dependent measures  
method: import raw response data for chosen task for both t1 and t2, calculate dv's for for both t's for different number of trials, calculate retest reliability point estimates for each of these dv's with different trial numbers  
candidates: angling risk task, columbia card task, dietary decision, hierarchical rule, keep track, probabilistic selection, psychological refractory period, two stage decision  

```{r warning=FALSE, message=FALSE}
#read t1 data
t1_hr = read.csv(gzfile(paste0(test_data_path,'Individual_Measures/hierarchical_rule.csv.gz'), 'rt'))

t2_hr = read.csv(gzfile(paste0(retest_data_path,'Individual_Measures/hierarchical_rule.csv.gz'), 'rt'))

#get only retest subjects
t1_hr = t1_hr[as.character(t1_hr$worker_id) %in% as.character(test_data$sub_id),]
t2_hr = t2_hr[as.character(t2_hr$worker_id) %in% as.character(retest_data$sub_id),]

#remove missed trials (rt == -1)
t1_hr = t1_hr %>% filter(rt != -1)
t2_hr = t2_hr %>% filter(rt != -1)

#dv's
#accuracy: mean correct
#average rt: median of correct trials only
#score: sum correct

calc_break_dvs = function(x, breaks=seq(0, 360, 10)[-1]){
  
  calc_dvs = function(x){
    out = x %>%
      summarise(acc = mean(correct),
                avg_rt = median(rt[correct == 1]))
                # score = sum(correct))
    return(out)
  }
...
# merge to get same number of rows for each subject at both time points
...
# trial_num_rel_df = data.frame(breaks=rep(NA, length(unique(t1_dvs$breaks))), acc_icc=rep(NA, length(unique(t1_dvs$breaks))), avg_rt_icc=rep(NA, length(unique(t1_dvs$breaks))), score_icc=rep(NA, length(unique(t1_dvs$breaks))))
trial_num_rel_df = data.frame(breaks=rep(NA, length(unique(t1_dvs$breaks))), acc_icc=rep(NA, length(unique(t1_dvs$breaks))), avg_rt_icc=rep(NA, length(unique(t1_dvs$breaks))))
for(i in 1:length(unique(t1_dvs$breaks))){
  ...
  # trial_num_rel_df$score_icc[i] = get_icc("score", tmp_t1_dvs, tmp_t2_dvs)
}
...
trial_num_rel_df %>%
  ...
  # ggplot(aes(breaks*10, value, color=factor(key, levels = c("acc_icc", "avg_rt_icc", "score_icc"), labels = c("Accuracy", "Average RT", "Score"))))+
  ggplot(aes(breaks*10, value, color=factor(key, levels = c("acc_icc", "avg_rt_icc"), labels = c("Accuracy", "Average RT"))))+
  geom_point()+
  geom_line()+
  theme_bw()+
  xlab("Number of trials")+
...
```{r}
rel_df[c(rel_df$dv %in% grep('hierarchical_rule', rel_df$dv, value = TRUE)),]
```
