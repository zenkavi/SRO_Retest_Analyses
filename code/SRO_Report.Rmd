...
get_var_breakdown <- function(dv_var, t1_df = retest_subs_test_data, t2_df = retest_data, merge_var = 'sub_id', sample='full', sample_vec){
  ...
  df = df %>% select(-dv, -sub_id)
  icc = ICC(df)
  var_breakdown = data.frame(subs = ICC(tmp)$summary[[1]][1,'Mean Sq'],
                             ind = ICC(tmp)$summary[[1]][2,'Mean Sq'],
                             resid = ICC(tmp)$summary[[1]][3,'Mean Sq'])
...
for(i in 1:length(numeric_cols)){
  ...
  rel_df[numeric_cols[i], 'var_subs'] <- get_var_breakdown('adaptive_n_back.mean_load.logTr')$subs
  rel_df[numeric_cols[i], 'var_ind'] <- get_var_breakdown('adaptive_n_back.mean_load.logTr')$ind
  rel_df[numeric_cols[i], 'var_resid'] <- get_var_breakdown('adaptive_n_back.mean_load.logTr')$resid
}
