...
References mentioned in the plot below can be found [here](https://docs.google.com/spreadsheets/d/1hcED4_rWSGSE8Td0aqT0kWqlLCBAzRNKBXj3tSram58/edit?usp=sharing).
...
```{r warning=FALSE, message=FALSE, fig.height=72, echo=FALSE}
ggplotly(fig_data %>%
           mutate(dv = factor(dv, levels = dv[order(task)]),
                  days_cutoff = ifelse(days < 60, days, 120),
                  days = paste(days, '\n reference:', reference)) %>%
           ...
           geom_point(aes(size=sample_size, shape = type, alpha = days_cutoff), color = '#00BFC4')+
...
# if you figure out how to make the panels for tasks and surveys separately you can use subplot() - a ggplotly function to plot them side by side
```
...
ggplotly(fig_data %>%
  mutate(dv = factor(dv, levels = dv[order(task)]),
         days_cutoff = ifelse(days < 60, days, 120),
         days = paste(days, '\n reference:', reference)) %>%
  filter(task_group == 'bis_bas_survey') %>%
  ggplot(aes(y = var, x = retest_reliability, label=days))+
