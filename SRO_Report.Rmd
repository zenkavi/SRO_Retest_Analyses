...
clean_test_data = cbind(sub_id = test_data$sub_id, as.data.frame(apply(test_data[, -which(names(retest_data) %in% c("sub_id"))], 2, remove_outliers)))
  ...
  if(tmp_col %in% logTr_vars$var){
    sign = logTr_vars$sign[which(tmp_col == logTr_vars$var)]
      if(sign == 'positive'){
        ...
        names(clean_retest_data)[i] = paste0(tmp_col, '.logTr')
      }
      else if(sign == 'negative'){
        clean_retest_data[,tmp_col] = neg_log(clean_retest_data[,tmp_col])
        names(clean_retest_data)[i] = paste0(tmp_col, '.ReflogTr')
      }
    }
}
```
