...
Our cleaning procedures includes two steps:  
1. removing outliers for each variables where outliers are defined as those who are greater than 2.5 IQR from median  
2. Log transforming variables with skew. In cleaning T1 data for the `meaningful_variables_clean.csv` variables for which skew could not be removed successfully were dropped. **This step is excluded here. Variables that are not successfully transformed will be noted but not dropped.**
