...
```{r}
tst_mng_cln <- read.csv('/Users/zeynepenkavi/Documents/PoldrackLabLocal/Self_Regulation_Ontology/Data/Complete_01-31-2017/meaningful_variables_clean.csv')

rtst_mng_cln <- read.csv('/Users/zeynepenkavi/Documents/PoldrackLabLocal/Self_Regulation_Ontology/Data/Retest_02-11-2017/meaningful_variables_clean.csv')

names(rtst_mng_cln)[which(names(rtst_mng_cln) %in% names(tst_mng_cln) == FALSE)]

names(tst_mng_cln)[which(names(tst_mng_cln) %in% names(rtst_mng_cln) == FALSE)]
```
...
summary(lmer(score ~ (1|sub_id)+factor(time),data=tmp))

```

```{r}
summary(aov(score ~ factor(time) + Error(sub_id),tmp))
```
