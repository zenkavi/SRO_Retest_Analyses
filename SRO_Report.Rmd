...
tmp %>% ggplot(aes(value, color=label_2, fill=label_3))+
...
Make data frame with standardized difference between two scores for each measure for each subject.
...
str(as.data.frame(t1_2_difference$difference))
```
...
The effect of number of days in between in the full model you'd want (potentially even including the interaction between the fixed effects)

```{r eval = FALSE}
mod = lmer(difference ~ dv + days_btw + (1|sub_id), data = t1_2_difference)

mod_fixed = as.data.frame(fixef(mod))
mod_fixed["days_btw",]
```

Does average standardized difference change with number of days between completion?
...
Box plots showing the distribution of difference scores for each task

```{r}
t1_2_difference %>%
  ggplot(aes(dv, difference))+
  geom_boxplot()+
  theme_bw()+
