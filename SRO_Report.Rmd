...
```{r}
rm(hddm_refits, transformed_hddm_refits, hddm_logTr_vars, transform_hddm_logTr_vars, replace_hddm_vars, hddm_fix_retest_subs_test_data)
```
...
```{r}
match_t1_t2 <- function(dv_var, t1_df = retest_subs_test_data, t2_df = retest_data, merge_var = 'sub_id'){
...
get_icc <- function(dv_var, t1_df = retest_subs_test_data, t2_df = retest_data, merge_var = 'sub_id'){
...
get_eta <- function(dv_var, t1_df = retest_subs_test_data, t2_df = retest_data, merge_var = 'sub_id'){
  df = match_t1_t2(dv_var, t1_df = t1_df, t2_df = t2_df, merge_var = merge_var)
...
get_sem <- function(dv_var, t1_df = retest_subs_test_data, t2_df = retest_data, merge_var = 'sub_id'){
  df = match_t1_t2(dv_var, t1_df = t1_df, t2_df = t2_df, merge_var = merge_var)
  mod = summary(aov(score~Error(sub_id)+time, df))
...
Here is a scatter plot of these variables but I'm not including it in the report for now because it is too large and because I want to get back to this once I refit the hddm's for retest participants only.

```{r warning=FALSE, message=FALSE, fig.height=72, eval=FALSE}
tmp = data.frame()

for(i in 1:nrow(time_effect_vars)){
  tmp = rbind(tmp, match_t1_t2(time_effect_vars$value[i]))
}
rm(time_effect_vars)

tmp = tmp %>%
  inner_join(icc_df[,c("dv", "eta_sq")],by="dv") 

tmp = tmp %>% 
  mutate(dv = factor(dv, levels = unique(tmp$dv[order(tmp$eta_sq)])))
...
In this section we label the measures based on the putative cognitive process they are intended to measure. For tasks this is mainly informed by Patrick's expertise/cognitive atlas and for surveys it is planned to be informed by the factor/IRT analyses. We aim to make contact with the literature to make the results more accesible to readers who have experience with our tasks but do not want to   
a. take away from the novelty of the main papers coming out of this project outlining the factor structure that this data unveils   
b. necessarily make prescriptive recommendations or theoretical arguments on why certain measures of certain processes are more reliable than others.
...
measure_labels %>%
  ...
  filter(task == 'task') %>%
  select(-measure_description, -notes, -variable_type, -task) %>%
  left_join(icc_df[,c("dv", "icc")], by='dv') %>%
  left_join(cor_df[,c("dv", "spearman")], by='dv') %>%
  separate(measure_type, c("label_1", "label_2", "label_3"), sep = ",") %>%
  gather(key, value, -dv, -icc, -spearman) %>%
  arrange(dv) %>%
  drop_na() %>%
  mutate(value = trim(value)) %>%
  filter(dv %in% c("dot_pattern_expectancy.bias", "go_nogo.bias") == FALSE)%>%
  ggplot(aes(icc))+
...
##### Difference versus all trials

#### Raw versus model fit

#### Condition versus all

### Surveys

Would this be a cmoparison of factor reliability versus item reliability?
