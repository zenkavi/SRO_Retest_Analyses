...
Bootstrapping everything at once locally would take around four hours.
```{r eval=FALSE}
as.data.frame(numeric_cols) %>%
  mutate(index = 1:n()) %>%
  group_by(index) %>%
  # rowwise()%>% #this works too but the output is nicer the other way
  do(plyr::rdply(2, bootstrap_relialibility(dv_var = as.character(.$numeric_cols), metric = c("icc", "spearman"))))
