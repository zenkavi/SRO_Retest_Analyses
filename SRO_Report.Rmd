...
render_this <- function(){rmarkdown::render('SRO_Report.Rmd', output_dir = '/Users/zeynepenkavi/Dropbox/PoldrackLab/SRO_Retest_Report', html_notebook(toc = T, toc_float = T))}
```
...
ggplotly(cor_df %>%
  ggplot(aes(spearman, fill=task, label=dv))+
...
Sort tasks by icc. What's going on with the negative icc's?
ICC(3,k) would be negative if the $MS_{error}$>$MS_{between}$

Plot their data scatter plots.
...
Compare these plots to the variables with highest icc
...
Mostly linear except for a bunch of tasks that have an icc ~ 0.
