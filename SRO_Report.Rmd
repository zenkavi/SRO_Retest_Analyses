...
measure_labels %>%
  ...
  gather(key, value, -dv, -icc, -spearman, -num_all_trials) %>%
  arrange(dv) %>%
  drop_na() %>%
  mutate(value = trim(value))
```

Does number of items in a trial have a significant effect on the average reliability of raw measures from a task?

```{r}
summary(lm(icc ~ num_all_trials))
```

```{r warning=FALSE, message=FALSE, eval=FALSE}
#Plot of distribution of icc's faceted by putative measure
measure_labels %>%
  mutate(dv = as.character(dv)) %>%
  filter(task == 'task') %>%
