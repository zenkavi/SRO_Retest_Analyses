...
Distribution of all Pearson correlations
```{r}
...
Distribution of all Spearman correlations
```{r}
...
names(dist_retest_data) <- c('not_normal')
dist_retest_data$dv <- row.names(dist_retest_data)
row.names(dist_retest_data) <- seq(1:nrow(dist_retest_data))
dist_retest_data = dist_retest_data %>% select(dv, not_normal)
```

Check distributions of test data (for subjects who have retest data - restricting here since there are the subsets of data that went in to the correlation matrix)
```{r}
dist_retest_subs_test_data <- as.data.frame(apply(retest_subs_test_data[,matching_dv_columns], 2, diff_from_normal))
names(dist_retest_subs_test_data) <- c('not_normal')
...
dist_retest_subs_test_data = dist_retest_subs_test_data %>% select(dv, not_normal)
```

What proportion is not normal?
```{r}
sum(dist_retest_data$not_normal)/nrow(dist_retest_data)
```

```{r}
sum(dist_retest_subs_test_data$not_normal)/nrow(dist_retest_subs_test_data)
```
...
A majority of them are drift diffusion parameters (but note that these vars are overrepresented to begin with).
```{r}
dist_retest_data[which(dist_retest_data$not_normal != dist_retest_subs_test_data$not_normal),]
```
...
```{r}
transform_dv_df = data.frame(dv = c(), transform = c(), not_normal = c())
#check if normal
diff_from_normal(retest_data[,2])
diff_from_normal(log(retest_data[,2]))
diff_from_normal(sqrt(retest_data[,2]))
