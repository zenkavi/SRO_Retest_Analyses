...
render_this <- function(){rmarkdown::render('SRO_Retest_Analyses.Rmd', html_notebook(toc = T, toc_float = T))}
options(digits = 4)
```
...
**I'm not sure what the right thing to do is regarding the cleaning of the test data for the retest subjects.**   
Should they be cleaned on the whole sample for the time they were collected on and then extracted (this should have an effect on outliers as well as transformations) or extracted and then cleaned. Here I am doing the latter but this seems to be resulting in more skewed variables for test data compared to the retest data so the later correlation matrix might not include all the variables we would like to look at.

```{r warning=FALSE}
clean_retest_subs_test_data = as.data.frame(apply(retest_subs_test_data[, matching_dv_columns], 2, remove_outliers))
clean_retest_subs_test_data = transform_remove_skew(clean_retest_subs_test_data, matching_dv_columns)
```

## Correlations on cleaned variables

Create the same list of correlations as before but this time on a smaller set of cleaned variables.
...
for(i in 1:length(clean_matching_dv_columns)){
  ...
  robustCor <- function(...) {
    obj<-try(cor(...), silent=TRUE)
    if (is(obj, "try-error")) return(NA) else return(obj)
  }
  
  tmp_pearson = robustCor(clean_retest_data[,clean_matching_dv_columns[i]], clean_retest_subs_test_data[,clean_matching_dv_columns[i]], method = 'pearson', use = "pairwise.complete.obs")
  
  tmp_spearman = robustCor(clean_retest_data[,clean_matching_dv_columns[i]], clean_retest_subs_test_data[,clean_matching_dv_columns[i]], method = 'spearman', use = "pairwise.complete.obs")
  
  clean_cor_df[clean_matching_dv_columns[i], 'pearson'] <- tmp_pearson 
  clean_cor_df[clean_matching_dv_columns[i], 'spearman'] <- tmp_spearman
...
Correlations could not be calculated for the following variables due to lack of variance.
...
Let's look at the distributions again: The distributions don't look terribly different but there are noticably fewer variables post cleaning. 
```{r warning=FALSE, message=FALSE}
cor_df %>%
  left_join(clean_cor_df, by='dv') %>%
  select(-task.y)%>%
  rename(pearson_preClean = pearson.x, spearman_preClean = spearman.x,
         pearson_postClean = pearson.y, spearman_postClean = spearman.y,
         task = task.x) %>%
  gather(key, value, -dv, -task) %>%
  separate(key, c('cor_type', 'clean_type'),sep = "_") %>%
  ggplot(aes(x=value, fill=factor(clean_type, levels = c('preClean', 'postClean'),
                                  labels = c('Before', 'After'))))+
  geom_histogram(position='identity', alpha = 0.5)+
  facet_wrap(cor_type~task)+
  theme_bw()+
...
clean_cor_df %>%
  ...
  summarise(mean_pearson = mean(pearson, na.rm=T),
            ...
            num_vars = sum(ifelse(is.na(pearson)==FALSE,1,0)))
```

**What changed most with cleaning?**    
Difference in correlation pre and post correlation for the variables that survived cleaning.  
Things that changed most seem to be RTs and parameter estimates.

```{r}
clean_cor_df %>%
  left_join(cor_df, by='dv') %>%
  mutate(pearson_diff = abs(pearson.y - pearson.x),
         spearman_diff = abs(spearman.y - spearman.x)) %>%
  select(dv, pearson_diff, spearman_diff) %>%
  arrange(-pearson_diff, -spearman_diff)
```
...
I'm not sure why there are so many more EZ variables than hddm variables.
...
cor_df%>%
  ...
  left_join(clean_cor_df[,c('spearman', 'dv')], by = 'dv') %>%
  gather(clean_type, spearman, -dv, -var_type, -var, -task) %>%
  mutate(clean_type = ifelse(clean_type == 'spearman.x','Before','After')) %>%
  ggplot(aes(spearman, fill = clean_type))+
  geom_histogram(position='identity', alpha = 0.5)+
  theme_bw()+
  facet_wrap(var_type~var)+
  scale_fill_discrete(name = 'Clean type')
  
```

Average reliabilities

1. Cleaning doesn't seem to make a huge difference (at least for Spearman correlations) except for raw accuracies and hddm non_decision  
2. There are many more EZ variabiles but the few hddm variables have slightly higher reliabilities (I'm not sure about the reason for the discrepancy between the number of variables)  
3. Raw accuracies seem to have the worst reliabilities on average while hddm parameters the highest  
...
cor_df%>%
  ...
  gather(clean_type, spearman, -dv, -var_type, -var, -task) %>%
  mutate(clean_type = ifelse(clean_type == 'spearman.x','Before','After')) %>%
  group_by(var_type, clean_type, var) %>%
  summarise(mean_spearman = mean(spearman, na.rm=T),
            sem_spear = sem(spearman),
            num_vars = sum(ifelse(is.na(spearman)==FALSE,1,0))) %>%
  arrange(var_type, -as.numeric(as.factor(clean_type)))
```

## Other variable types

I planned to look at differences between distributions of reliabilities for other variables types as well but I am not sure what these categories should be so I am deferring this for further discussions. My earlier notes suggested differences between other model parameters versus raw variables; subtraction versus basic variables; single versus multiple variables.I'm hoping we can come up with a more systematic definition that I can automatize or at least restrict to certain groups of variable instead of going through all 600+ variables and manually labeling them.

# Replicating the factor structure

## Confirm comparability of retest and rest sample

First do a t-test on all measures of T1 for subjects with and without retest to confirm that these two samples do not differ from each other significantly
```{r}
ttest_df = data.frame(t_stat = rep(NA, length(matching_dv_columns)),
                      p_val = rep(NA, length(matching_dv_columns)))

row.names(ttest_df) <- matching_dv_columns

for(i in 1:length(matching_dv_columns)){
  ttest_df[matching_dv_columns[i], 't_stat'] <- pairwise.t.test(retest_subs_test_data[,matching_dv_columns[i]], rest_subs_test_data[,matching_dv_columns[i]], p.adjust.method = 'fdr')$statistic
  
  ttest_df[matching_dv_columns[i], 'p_val'] <- t.test(retest_subs_test_data[,matching_dv_columns[i]], rest_subs_test_data[,matching_dv_columns[i]], p.adjust.method = 'fdr')$p.value
}
...
corrected_ttest <- function(df1, df2, col){
  ...
  tmp = as.data.frame(cbind(x,y))
  
  tmp = tmp %>% 
    gather(key, value)
  
  return(pairwise.t.test(tmp$value,tmp$key, p.adjust.method = 'fdr'))
}
```
...
43 variables. None of them are very strongly different though.

```{r}
ttest_df %>%
  filter(p_val<0.05) %>%
  select(dv, task, t_stat, p_val)
```

Do this again with cleaned data.  
Clean the T1 data for the subsets first.  
**Here again I'm not sure if I should clean before or after splitting. Before it seemed appropriate to clean subjects with retest data after separating them from the remaining subjects but here it might be better to clean first and then separate. But then it would differ from what was done for earlier anaylses and it seems like the kind of thing that should be kept consistent.**

Clean T1 for subjects with retest data (doing it again because this df was subsetted before)
...
Clean T1 for rest of the subjects

```{r warning=FALSE}
clean_rest_subs_test_data = as.data.frame(apply(rest_subs_test_data[, matching_dv_columns], 2, remove_outliers))
clean_rest_subs_test_data = transform_remove_skew(clean_rest_subs_test_data, matching_dv_columns)
```

Get numeric dv's from both cleaned datasets to compute t-tests on.
```{r}
clean_all_columns <- unique(c(names(clean_rest_subs_test_data), names(clean_retest_subs_test_data)))

clean_matching_columns <- c()
for(i in 1:length(clean_all_columns)){
  if(clean_all_columns[i] %in% names(clean_rest_subs_test_data) & clean_all_columns[i] %in% names(clean_retest_subs_test_data)){
    clean_matching_columns <- c(clean_matching_columns, clean_all_columns[i])
  }
}
...
for(i in 1:length(clean_matching_columns)){
  if(is.numeric(clean_rest_subs_test_data[,clean_matching_columns[i]]) & is.numeric(clean_rest_subs_test_data[,clean_matching_columns[i]])){
    clean_matching_dv_columns <- c(clean_matching_dv_columns, clean_matching_columns[i])
  }
}
```

Compute t-tests for cleaned variables.
```{r}
clean_ttest_df = data.frame(t_stat = rep(NA, length(clean_matching_dv_columns)),
                      p_val = rep(NA, length(clean_matching_dv_columns)))

row.names(clean_ttest_df) <- clean_matching_dv_columns

for(i in 1:length(clean_matching_dv_columns)){
 
  robustTTest <- function(...) {
    obj<-try(t.test(...), silent=TRUE)
    if (is(obj, "try-error")) return(obj = data.frame(statistic = NA, p.value=NA)) else return(obj)
  }
  
  tmp = robustTTest(clean_retest_subs_test_data[,clean_matching_dv_columns[i]], clean_rest_subs_test_data[,clean_matching_dv_columns[i]])
  
  clean_ttest_df[clean_matching_dv_columns[i], 't_stat'] <- tmp$statistic 
  clean_ttest_df[clean_matching_dv_columns[i], 'p_val'] <- tmp$p.value
    }

clean_ttest_df$dv = row.names(clean_ttest_df)
row.names(clean_ttest_df) = seq(1:nrow(clean_ttest_df))
clean_ttest_df$task = 'task'
clean_ttest_df[grep('survey', clean_ttest_df$dv), 'task'] = 'survey'
```

Variables for which t-tests cannot be computed due to lack of variance.
```{r}
clean_ttest_df %>%
  filter(is.na(t_stat)) %>%
  select(dv)
...
Distribution of t-tests for the cleaned variables with sufficient variance
```{r warning=FALSE, message=FALSE}
clean_ttest_df %>%
  filter(!is.na(t_stat)) %>%
ggplot(aes(t_stat)) +
...
**Which variables are significantly different between the two samples on cleaned data (those with and without retest data for T1)?**

More variables (that are thought to have been properly cleaned for these analyses) are significantly different between the two samples.   
**Not sure what to do with this especially for the next set of analyses.**
...
Step 2: Efa on the people you don't have retest on and then predict how well this holds up for people w retest on t1 and on t2.  

I'm restricting the factor analyses to the meaningful variables for now (instead of the 600+ exhaustive variables) to keep it manageable. I haven't consulted other factor analyses that have been done on the full dataset in doing this but will be happy to adjust as necessary.
...
For now I'm just removing these variables but will have to do re-do them once I resolve why they weren't in both datasets to begin with (The answer will be a combination of not having run the most recent analysis script on some tasks and dropped variables as a result of cleaning)
```{r}
...
Trying to use `lavaan` first. It's not happy with me.
```{r}
unrotated <- try(efaUnrotate(rest_subs_test_mngf_z, nf=3, varList=names(rest_subs_test_mngf_z)[-1], estimator="mlr"))
...
# Invited vs. completed retest

T1 comparison for people who came back vs didn't (of all that are invited)

- Ignore Pearson for now and just focus on Spearman
- Eventually want to get to interclass correlations (between versus within group variability - using anova)
- Clean everything on the whole sample and extract from that. Keep a record of everything that was transformed and whether it was normal at the end
